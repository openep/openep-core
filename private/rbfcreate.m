...
% Modified by Adam Connolly on 8/9/17 to include RBF derivatives
...
%************************************************************************** 
    function [u,du]=rbfphi_linear(r, const)
        u=r;
        du=1./r;
    end

    function [u,du]=rbfphi_cubic(r, const)
        u=r.*r.*r;
        du=3*r;
    end

    function [u,du]=rbfphi_gaussian(r, const)
         c2=const*const;
        u=exp(-0.5*r.*r/c2);
        du=-u./c2;
    end

    function [u,du]=rbfphi_multiquadrics(r, const)
        c2=const*const;
        u=sqrt(1+r.*r/c2);
        du=1./(c2*u);
    end

    function [u,du]=rbfphi_thinplate(r, const)
        u=r.*r.*log(r+1);
        du=r./(r+1)+2*log(r+1);
    end
