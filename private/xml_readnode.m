function [tree, RootName] = xml_readnode(RootNode, Pref)
...
Debug           = true;  % show specific errors (true) or general (false)?
tree            = [];
RootName        = [];
...
if (~all(~isempty(s)))          % if nothing left than this is probably a number
  ...
  if (option==1)                % the 'smart' option
    try                         % try to convert to a date, like 2007-12-05
      option = 2; % modified by NL - this is very slow so assum it isn't a date
