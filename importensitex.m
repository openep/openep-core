function info = importensitex(varargin)
...
% necessary.    
    ...
    for iDET=1:numel(allDataExportTypes)
        isMatch = false(numel(info), 1);
        thisDET = allDataExportTypes{iDET};
        for iIn=1:numel(info)
            if isempty(info{iIn})
                isRead(iIn) = true;
            elseif isfield(info{iIn}, 'dataElement')
                infoDET = info{iIn}.dataElement;
                infoDET = translateDataExportElement(infoDET);
                if strcmp(infoDET,thisDET)
                    isMatch(iIn) = true;
                end
            end
        end
        ...
        if numel(ind)>0
            ...
            newInfo = setfield(newInfo,thisDET,temp); %#ok<SFLD>
            isRead(isMatch)=true;
        end
    end
    if any(~isRead)
